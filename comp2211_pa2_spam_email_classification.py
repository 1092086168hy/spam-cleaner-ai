...
Original file is located at
    https://colab.research.google.com/drive/1yJwVdFwkWQ3Yw2-NkACkagd0N7TBhHGa
...
DATA_PATH = 'full_spam_dataset.json'
...
def create_vocabulary(cleaned_emails):
    ...
    list_of_words=cleaned_emails.split()
    word_counts=Counter(list_of_words)
    frequency=[word for word,count in word_counts.items() if count>=5]
    vocab=sorted(set(frequency))
...
def encode_emails(cleaned_emails, vocab, word_to_index):
    ...
    for clean_email in cleaned_emails:
      word=clean_email.split()
      every_word_to_index=[]
      for i in word:
        if i not in word_to_index:
          every_word_to_index.append(0)
        else:
          every_word_to_index.append(word_to_index[i])
      encoded_emails.append(every_word_to_index)
...
def pad_encoded_emails(encoded_emails, max_len = 300):
    ...
    for index,word in enumerate(encoded_emails):
      final_len=min(max_len,len(word))
      padded_emails[index,0:final_len]=word[0:final_len]
